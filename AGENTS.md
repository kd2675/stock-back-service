...
- `POST /api/stock/v1/markets/liquidity-mandates/{symbol}` (`ADMIN`, 실행 중에도 LP 계정과 시드 자산을 준비하고 다음 안전 개장일 활성화를 예약)
...
- `POST /api/stock/v1/markets/liquidity-mandates/{symbol}/resume` (`ADMIN`, 중단 상태를 유지한 채 다음 안전 개장일 LP 재개를 예약)
...
- `POST /api/stock/v1/markets/underwriting-contracts/{contractId}/supply/activate` (`ADMIN`, 현재 계약 상태를 유지한 채 다음 안전 개장일 유한·수동 매도 전용 인수재고 공급 활성화를 예약)
