...
- `POST /api/stock/v1/markets/institution-portfolios` (`ADMIN`, 일시정지 장전에서 선택한 기관 1개만 주문 없는 SHADOW로 생성)
...
- `POST /api/stock/v1/markets/liquidity-mandates/{symbol}` (`ADMIN`, 일시정지 장전에서 선택한 종목의 LP 계정·계약·시드 자산만 SHADOW로 준비)
