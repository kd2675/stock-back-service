...
- `POST /api/stock/v1/markets/liquidity-mandates/{symbol}` (`ADMIN`, 일시정지 장전에서 해당 종목 레거시 호가 취소·설정 중지·시드 자산 이전·LP LIVE 생성을 한 트랜잭션으로 처리하며, 역할 분리형 신규 상장은 다음 장 개장 대상으로 활성화)
