...
2. `PAID_IN_CAPITAL_INCREASE`
   ...
   - 현재는 `SHAREHOLDER_ALLOCATION`과 `PUBLIC_OFFERING`만 지원한다.
   - 주주배정은 기준 snapshot 권리, 일반공모는 전체 잔여수량, 두 방식 모두 청약 현금 예치와 실제 청약수량만큼의 상장을 별도 entitlement 원장으로 처리한다.
...
이 7개는 이미 구현된 범위이며, 초기 단계에서는 여기서 기업 이벤트 구현을 멈추는 것이 맞다. 다음 작업은 새 이벤트 추가가 아니라 기존 이벤트의 기준일, 단주 정책, 열린 주문 정책, 테스트를 더 견고하게 만드는 쪽이 우선이다.
...
- 유상증자 자체의 주주배정/일반공모 청약과 미행사 만료는 현재 구현한다.
- 권리를 제3자에게 매매·양도하는 별도 증서/종목 모델과 권리 가격 형성은 아직 구현하지 않는다.
