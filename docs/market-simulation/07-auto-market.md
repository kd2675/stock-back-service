...
- max_order_quantity
- order_ttl_seconds
- primary_*_pressure_bias: 네 차례 주 압력의 항목별 분포 최빈값, -100~100
...
4. 현재 시각의 주 압력 구간과 30분 보조 압력 구간을 읽고, 없으면 편향을 반영해 -100~100 값을 생성한다.
5. 주 압력 60%와 보조 압력 40%를 항목별로 합성한다.
...
10. 현재가, 최우선 매수/매도 호가, tick size를 참고해 지정가를 만든다.
