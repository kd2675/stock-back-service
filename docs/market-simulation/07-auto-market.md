...
10. 현재가, 최우선 매수/매도 호가, tick size를 참고해 지정가를 만든다. 일반 자동 참여자는 가격 압력을 현재가 비율로 반영하되 기본 0.6%, 변동성 반영 후 0.8% 이내로 제한하고, 상승 압력에서는 매수 교차 확률을 높이고 매도 교차 확률을 낮춘다. 하락 압력은 반대로 작동한다.
