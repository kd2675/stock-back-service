...
초기 프로젝트에서 어떤 기업 이벤트까지 구현할지는 `15-corporate-action-scope.md`를 우선 기준으로 본다. 실제 시장 이벤트가 많아도, 단주/권리/상장폐지처럼 원장 정책이 정해지지 않은 이벤트는 초기 구현에 넣지 않는다.
...
3. 상장폐지는 열린 주문, 보유수량, 가격 표시 정책을 먼저 정한다.
