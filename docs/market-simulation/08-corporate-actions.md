...
- `GET /api/stock/v1/markets/order-book-instruments/{symbol}/cash-dividend-guidance`는 완료된 배당 이력, 직전 실제 지급액·권리 수량, 액면분할 보정 주당 배당금, 최근 완료 전체 장마감의 보유수량을 경량 조회한다.

관리자 권유값:

- 권유값은 재무 적정성을 보장하지 않는 `과거 기준 권유값`이며, 입력값을 자동 확정하지 않는다.
- 가격 기준은 최근 장마감 종가를 우선하고 없을 때만 현재가를 사용한다.
- 이전 주당 배당금은 이후 완료된 액면분할 비율로 현재 주식 기준에 맞춘다.
- 예상 지급액은 최근 완료 전체 장마감 보유수량 기준과 발행주식 전량 기준 상한을 구분한다.
- 직전 대비 주당 배당금·배당수익률·보유기준 지급 규모를 `100%`, `200%`, `500%+` 구간으로 비교하며, 2배·5배 초과는 경고만 하고 입력을 차단하지 않는다.
