...
4. 참여자별-종목별 intensity 기준으로 주문 수를 정한다.
5. 자동 참여자, 매수/매도 방향, 수량, 가격을 정한다.
6. 실제 `stock_order`에 `ORDER_BOOK`, `LIMIT` 주문을 만든다.
7. `StockBatchJobService.runAutoMarket`은 자동 주문 생성 후 `InternalOrderBookExecutionService.executeEligibleOrders`를 같이 실행한다.
...
3. 주문 생성 정책은 `placeAutoOrders`, `chooseSide`, `createAutoPrice` 순서로 본다.
