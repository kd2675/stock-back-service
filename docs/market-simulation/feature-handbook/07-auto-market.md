...
5. 참여자별-종목별 주문 활동 강도와 최신 보고서 점수를 각각 읽는다. 명시 설정이 없으면 활동 강도 5를 쓰며 보고서 점수로 활동 강도를 바꾸지 않는다.
6. 참여자 `profile_type`에 맞는 `*Behavior` 클래스가 압력·주문장·계좌 상태를 반영해 주문 수, 매수/매도 방향, 수량 상한, TTL을 정한다.
7. `AutoMarketService`가 시장·보고서 방향 압력에 프로필별 가격 압력 민감도를 적용해 가격을 만들고 현금/보유 제약을 확인한다.
...
- 일반 자동 참여자의 가격 압력은 현재가 대비 비율로 호가 중심을 옮기며 기본 최대 이동률은 0.6%, 변동성 반영 후 절대 상한은 0.8%다. 중립 압력에서는 매수·매도를 중심가 양쪽에 두고, 상승 압력은 매수 교차 확률을 높이고 매도 교차 확률을 낮추며 하락 압력은 반대로 작동한다. 생성 단계는 시장성 지정가를 한 틱 밖으로 되돌리지 않고 실제 동일 계좌 또는 동일 `self_trade_group_id` 자기체결은 체결 엔진에서 막는다.
