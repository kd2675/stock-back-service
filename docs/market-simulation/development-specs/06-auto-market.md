...
10. 현재가, 전일종가, 최우선 매수/매도, 호가 잔량, 평균단가, 종목별 tick size 기준으로 자동 주문 방향과 가격을 만든다. 일반 자동 참여자의 비교차 호가는 가격 압력을 현재가 비율로 반영하고 기본 0.6%, 변동성 반영 후 최대 0.8%로 제한한다. 상대 호가 교차 확률은 상승 압력에서 매수를 높이고 매도를 낮추며 하락 압력에서는 반대로 적용한다. 시장조성 프로필의 수동 호가 정책은 유지한다.
...
- 한 실행에서 여러 주문을 계획할 때 계좌 가용 현금·보유 수량·자기 최우선 호가와 종목 미체결 잔량은 이미 계획한 주문을 메모리에서 반영해 다음 판단에 사용한다. 이를 위해 주문마다 DB를 다시 조회하지 않는다.
- 교차 추첨에 실패한 일반 자동 참여자 주문은 상대 최우선 호가를 넘지 않으며, 가격제한폭 때문에 합법적인 비교차 가격이나 자기체결 회피 가격을 만들 수 없으면 `INVALID_PRICE`로 탈락한다.
