...
- 최신 활성 보고서 조회
- 보고서 이벤트 이력 조회
- 최신 활성 보고서 점수를 자동장 가격 방향 압력에 반영
...
- 참여자 활동 강도는 보고서 점수와 섞지 않고 주문 우선순위·건수 기준으로 유지한다.
- 보고서 점수는 `AutoMarketConfig.reportPricePressure()`로 방향화한 뒤 뉴스 민감도와 프로필 가격 압력 민감도를 거쳐 호가에 반영한다.
- 보고서가 없으면 보고서 방향 압력만 제외한다.
...
- 보고서 점수가 있어도 참여자 활동 강도와 주문 건수 기준은 그대로다.
- 호재·악재 보고서는 뉴스 민감도와 가격 압력 민감도에 따라 호가 방향에 반영된다.
- 보고서가 없으면 보고서 방향 압력이 0이다.
...
5. 자동장 활동 강도 비회귀와 보고서 방향 압력 계산을 테스트한다.
