...
2. 종목별 주·보조 압력 분포 편향과 참여자별 주문 활동 강도를 구분해 config에 반영한다.
3. 주 랜덤 1~4회 가중치, 필수 06시와 선택된 09·12·15시 갱신 슬롯, 30분 보조 구간, 70:30 합성 계약을 함께 수정한다.
4. 자동 주문이 ready-symbol 큐를 거쳐 사용자 주문과 같은 체결 worker를 타는지 테스트한다.
