...
- `POST /api/stock/v1/markets/liquidity-mandates/{symbol}` (`ADMIN`, 일시정지 장전에서 종목별 레거시 미체결 주문 취소·기존 공급 설정 비활성화·시드 자산 이전·전용 LP LIVE 생성을 한 트랜잭션으로 처리. 역할 분리형 신규 상장은 같은 커밋에서 다음 장 개장 대상으로 활성화)
