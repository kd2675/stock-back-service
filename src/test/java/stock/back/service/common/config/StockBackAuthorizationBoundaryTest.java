...
class StockBackAuthorizationBoundaryTest {
    ...
    void setUp() {
        ...
        ensureAutoParticipantBehaviorStateTables();
        ensureMarketRoleTables();
    ...
    private void ensureAutoParticipantBehaviorStateTables() {
        jdbcTemplate.execute("""
        ...
        jdbcTemplate.execute("""
                create table if not exists stock_auto_participant_funding_budget (
                    ...
                    updated_at timestamp not null,
                    unique (account_id, budget_type, source_key)
                )
                """);
        jdbcTemplate.execute("""
    ...
    private void ensureMarketRoleTables() {
        jdbcTemplate.execute("""
                create table if not exists stock_market_participant (
                    ...
                    self_trade_group_id varchar(80) not null unique,
                    created_at timestamp not null,
                    updated_at timestamp not null
                )
                """);
        ...
        jdbcTemplate.execute("""
                ...
                """);
        jdbcTemplate.update("""
                merge into stock_market_participant(
                    participant_code, display_name, participant_type, status,
                    self_trade_group_id, created_at, updated_at
                )
                key(participant_code)
                values (
                    'SYSTEM_CUSTODY', '기본 시스템 보관기관',
                    'SYSTEM_CUSTODY', 'ACTIVE', 'SYSTEM_CUSTODY:DEFAULT', ?, ?
                )
                """, LocalDateTime.now(), LocalDateTime.now());
    }

    private void cleanAdminMarketTestData() {
        jdbcTemplate.update("delete from stock_security_allocation_ledger where symbol like 'ZQAUTH%'");
        jdbcTemplate.update("""
                delete from stock_market_participant_account
                 where account_id in (
                       select id
                         from stock_account
                        where user_key like 'stock-issuance-%zqauth%'
                 )
                """);
                ...
                delete from stock_auto_participant_share_return
                 where withdrawal_id in (
                       select id
                        ...
                        where participant_user_key like 'stock-auto-auth%'
                 )
                """);
        jdbcTemplate.update("delete from stock_auto_participant_withdrawal where participant_user_key like 'stock-auto-auth%'");
        ...
        jdbcTemplate.update("delete from stock_account where user_key like 'stock-issuance-%zqauth%'");
    ...
    void createOrderBookInstrument_adminPrincipalHeaders_isAllowed() throws Exception {
        ...
        Long floatCustodyHoldingCount = jdbcTemplate.queryForObject(
                """
                select count(*)
                  from stock_holding h
                  join stock_account a on a.id = h.account_id
                 where h.symbol = 'ZQAUTH01'
                   and a.user_key = 'stock-issuance-float-zqauth01'
                   and a.participant_category = 'SYSTEM_CUSTODY'
                   and h.quantity = 50000
                   and h.reserved_quantity = 0
                   and h.average_price = 70000.00
                """,
                Long.class
        );
        Long lockupCustodyHoldingCount = jdbcTemplate.queryForObject(
                """
                select count(*)
                  from stock_holding h
                  join stock_account a on a.id = h.account_id
                 where h.symbol = 'ZQAUTH01'
                   and a.user_key = 'stock-issuance-lockup-zqauth01'
                   and a.participant_category = 'SYSTEM_CUSTODY'
                   and h.quantity = 50000
                   and h.reserved_quantity = 0
                   and h.average_price = 70000.00
        ...
        assertThat(floatCustodyHoldingCount).isEqualTo(1L);
        assertThat(lockupCustodyHoldingCount).isEqualTo(1L);
    ...
    @ValueSource(strings = {
            ...
            "POST /api/stock/v1/markets/institution-portfolios/1/cash-adjustments",
            "PATCH /api/stock/v1/markets/institution-portfolios/1/policy",
            ...
            "PATCH /api/stock/v1/markets/liquidity-mandates/ZQAUTH01/policy",
            "POST /api/stock/v1/markets/liquidity-mandates/ZQAUTH01/suspend",
            "POST /api/stock/v1/markets/liquidity-mandates/ZQAUTH01/resume",
