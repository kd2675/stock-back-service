...
class StockUserServiceTest {
    ...
    void getMyProfile_authUserExists_returnsAuthUserAndAccount() {
        ...
        StockAccount account = StockAccount.open("user-1", new BigDecimal("10000000"));
        UserDto user = UserDto.builder()
                ...
                .role("USER")
                .build();
        when(accountService.getOrOpenAccount("user-1")).thenReturn(account);
        when(accountService.toResponse(account)).thenReturn(new AccountResponse("user-1", new BigDecimal("10000000"), new BigDecimal("10000000")));
